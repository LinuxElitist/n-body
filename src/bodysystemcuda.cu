                ...
                float deltaTime, float damping, int numTiles)
{
	...
	{
		sharedPos[i + threadIdx.x].x = 0.0f;
		sharedPos[i + threadIdx.x].y = 0.0f;
		sharedPos[i + threadIdx.x].z = 0.0f;
		sharedPos[i + threadIdx.x].w = 0.0f;
	}
	//typename vec4<T>::Type *sharedVel = SharedMemory<typename vec4<T>::Type>();
//    typename vec4<T>::Type position = oldPos[deviceOffset + index];
	sharedPos[index].x = oldPos[deviceOffset + index].x;
	sharedPos[index].y = oldPos[deviceOffset + index].y;
	sharedPos[index].z = oldPos[deviceOffset + index].z;
	sharedPos[index].w = oldPos[deviceOffset + index].w;
	__syncthreads();
	/*sharedVel[index].x = vel[deviceOffset + index].x;
	sharedVel[index].y = vel[deviceOffset + index].y;
	sharedVel[index].z = vel[deviceOffset + index].z;
	sharedVel[index].w = vel[deviceOffset + index].w;
	__syncthreads();*/
...
/*    position.x += velocity.x * deltaTime;
    ...
    newPos[index].x += velocity.x * deltaTime;
    newPos[index].y += velocity.y * deltaTime;
    newPos[index].z += velocity.z * deltaTime;


    // store new position and velocity
    //newPos[deviceOffset + index] = position;
    sharedPos[deviceOffset + index].x = newPos[index].x;
    sharedPos[deviceOffset + index].y = newPos[index].y;
    sharedPos[deviceOffset + index].z = newPos[index].z;
    sharedPos[deviceOffset + index].w = 1.0f;//mass
	__syncthreads();
    /*sharedVel[deviceOffset + index].x    = velocity.x;
    sharedVel[deviceOffset + index].y    = velocity.y;
    sharedVel[deviceOffset + index].z    = velocity.z;
    sharedVel[deviceOffset + index].w    = 1.0f; //inverse mass*/
...
//        int sharedMemSize = blockSize * 4 * sizeof(T); // 4 floats for pos
        ...
        int numBlocks = CONCURRENT_BLOCKS;
        int numTiles = numBlocks;
        integrateBodies<T><<<numBlocks, BLOCK_SIZE, NUM_BODIES>>>
